...
- **A workflow that pastes its method inline must turn on the new `Define Method Inline` toggle.** The toggle defaults to **off** (stored-method-first), and the fields it holds are only read when it is on — so an existing workflow's pasted bundle is ignored until you flip it, and the node fails fast with *"Nothing to run"* rather than starting anything. One click per node. The stored parameter shape also changed (`{ bundle: [{ content }] }` instead of `string[]`); the old shape is still read, so nothing is lost — only the toggle needs setting.
...
- **`Python Files`: run a method whose custom PipeFunc Python travels with it.** Paste the method into `MTHDS Bundles`, add its `funcs/*.py` / `structures/*.py` / `requirements.txt` under `Python Files`, and the node ships them together as one method bundle.
  ...
  Validated locally rather than by round-trip: `Python Files` with nothing pasted in `MTHDS Bundles` (Python is not a method), and unsafe paths the runner would reject (absolute, `..` traversal, backslashes, Windows drive forms). **Custom Python requires a sandbox-hosted runner** — the hosted Pipelex API is one; a bare self-hosted `pipelex-api` refuses `.py` rather than importing untrusted code in-process. Stated on the field and in the usage guide.
